...
if L then
	L.custom_off_always_show_casts = "Always show a target for |cff71d5ffNature's Wrath|r/|cff71d5ffWater Bolt|r"
end
--------------------------------------------------------------------------------
-- Initialization
--
...
function mod:OnBossEnable()
	-- Life Warden Gola
	self:Log("SPELL_CAST_START", "WaterBolt", 168092)
	...
	-- Earthshaper Telu
	self:Log("SPELL_CAST_START", "NatureWrath", 168040)
...
function mod:OnEngage()
	...
	revitalizeCount = 1
	toxicBloomCount = 1
	self:CDBar(427498, 1.1) -- Torrential Fury
	self:CDBar(427459, 5.1) -- Toxic Bloom
	self:CDBar(427510, 12.1) -- Noxious Charge
	self:CDBar(427509, 25.5) -- Terrestrial Fury
...
do
	local function printTarget(self, name, guid)
		self:TargetMessage(168040, name, "Personal", "None")
	end

	function mod:NatureWrath(args)
		if mod:GetOption("custom_off_always_show_casts") then
...
do
	local function printTarget(self, name, guid)
		self:TargetMessage(168092, name, "Personal", "None")
	end
...
function mod:NoxiousCharge(args)
	...
	if self:Tank() then
		self:Say(427510)
		self:SayCountdown(427510, 4, 8, 3)
