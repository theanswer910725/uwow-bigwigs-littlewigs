...
function mod:OnBossEnable()
	...
	self:Log("RANGE_DAMAGE", "PullDmg", "*")
	self:Log("SWING_DAMAGE", "PullDmg", "*")
	self:RegisterTargetEvents("CheckPull")
	self:RegisterEvent("UNIT_COMBAT", "CheckPull")
	self:RegisterEvent("UNIT_THREAT_LIST_UPDATE", "CheckPull")
	self:RegisterEvent("UNIT_HEALTH_FREQUENT", "CheckPull")
...
do
		...
		if next(players) then
			...
			for player in pairs(players) do
				if UnitInParty(player) then -- don't announce players from previous groups
					...
					listname = listname .. self:UnitName(player) .. ", "
				end
			end
			if list:len() > 0 then
	...
	local function announceUsabletarget(self, id, item)
		self:Sync("itemAvailabletarget", id)
		local players = {} -- who can use the item
		local icons = {}
		if item.professions then
			for profIcon, requiredSkill in pairs(item.professions) do
		...
		if next(players) then
			...
			if list:len() > 0 then
				message = message .. " - ".. L.usableBy:format(list:sub(0, list:len()-2))
				if self:GetOption("custom_on_announcement") and counttargets > 1 then
					sendChatMessage(name, L.usableBy:format(listname:sub(0, listname:len()-2)))
					self:Message("announce_buff_items", "Neutral", "Info", message, false)
				end
			end
		end
	...
	function mod:BigWigs_BossComm(_, msg, data, sender)
		...
		elseif msg == "itemAvailabletarget" then
			local id = tonumber(data)
			local item = buffItems[id] or guardItems[id]
			if item then
				usableFoundtarget(self, id, item)
			end
