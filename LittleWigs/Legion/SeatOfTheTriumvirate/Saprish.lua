...
function mod:OnEngage()
	self:Bar(245802, 4) -- Ravaging Darkness
	if self:Mythic() then
		self:Bar(248831, 5.5) -- Dread Screech
	end
	self:Bar(245873, 9) -- Void Trap
	self:Bar(247206, 12.5) -- Overload Trap
	self:Bar(247245, 22.5) -- Umbral Flanking
end
...
do
	...
	function mod:RavagingDarknessDamage(args)
		if self:Me(args.destGUID) then
			...
			if t-prev > 1.5 then
				prev = t
				self:Message(245802, "Personal", "Alert", CL.underyou:format(args.spellName))
			end
		end
	end
end
